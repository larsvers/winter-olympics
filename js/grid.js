...
function makeGrid() {
  ...
  databind(data.nations_grid[startChapter]); // ...then update the databind function
		...
		var enterSel = join.enter()
	  	...
	  	.attr('y', function(d,i) {
  			var y0 = Math.floor(i/20) % 20; // increases by 1 every 20
				...
				// console.log('d.x', Math.round(d.x), 'd.y', Math.round(d.y));
  	  	return d.y;
	  	})
			.attr('width', 0)
			.attr('height', 0);
	...
	updateGrid = function(chapter) {
		...
		// console.log(data.nations_grid[chapter]);
	...
	d3.select('.main-canvas').on('mousemove', function() {
		...
		// console.log('Mouse x', Math.round(mouseX), 'Mouse y', Math.round(mouseY));
		// console.log('canvasPos', canvasPos, 'tip', tip, 'tipDim', tipDim, 'arrowDim', arrowDim)
		...
		if (nodeData) { // if we found some data under our mouse write a tooltip
 			...
 			// console.log(nodeData.nation, 'x', Math.round(nodeData.x));
			...
			if (tipDim.width/2 < nodeData.x) { // if the tip fits in the center
				...
				// console.log('tipDim.width/2', tipDim.width/2, 'nodeData.x', nodeData.x, 'left');
				// console.log('node position within canvas', nodeData.y,'\n + canvas position relative to window', Math.round(canvasPos.top),'\n - height of tooltip', tipDim.height,'\n - border-width of triangle', arrowDim);
